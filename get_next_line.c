...
/*   Updated: 2025/02/19 17:59:08 by mgrandia         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	while (1)
	{
		i = process_buffer(&left, &line, fd, flag);
		if (i < 0)
		{
			flag = 1;
			if (ft_process_no_newline(&left, &line, fd, flag) == -1)
				return (line);
		}
		else
		{
			if (line)
				free(line);
			line = ft_memcpy(left, i + 1);
			ft_process_with_newline(&left, i);
			return (line);
		}
	}
}
